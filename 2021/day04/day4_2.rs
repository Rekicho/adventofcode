...
fn add_called_number(card: &mut Vec<Vec<(u32, bool)>>, called_number: u32) {
    for line in card {
        *line = line
            .into_iter()
            .map(|(num, called)| (*num, *called || *num == called_number))
            .collect();
    }
}

fn check_win(card: Vec<Vec<(u32, bool)>>) -> bool {
    let completed_lines: usize = card
        .clone()
        .into_iter()
        .filter(|line| {
            line.into_iter()
        ...
        .count();
    let completed_cols: usize = (0..card.clone().len())
        .filter(|i| {
            card.clone()
                .into_iter()
...
fn sum_uncalled(card: &Vec<Vec<(u32, bool)>>) -> u32 {
    card.into_iter().fold(0, |acc, line| {
        acc + line.into_iter().fold(
            0,
            |sum, (number, called)| if *called { sum } else { sum + number },
        )
    })
}
...
fn main() {
    ...
    for called_number in called {
        let winners: Vec<Vec<Vec<(u32, bool)>>> = cards
            .drain_filter(|card| {
