...
fn align(current: &Vec<Vec<i32>>, candidate: &Vec<Vec<i32>>) -> (bool, Vec<Vec<i32>>, Vec<i32>) {
...
fn manhattan(left: &Vec<i32>, right: &Vec<i32>) -> i32 {
...
fn main() {
    ...
    let mut scanners_pos: Vec<Vec<i32>> = Vec::from([Vec::from([0, 0, 0])]);
    ...
    while !aligned.is_empty() {
        ...
        for candidate in candidates {
            let (is_aligned, updated, scanner_pos) = align(&current, &candidate);

            if is_aligned {
                aligned.push_back(updated);
                scanners_pos.push(scanner_pos);
            } else {
    ...
    for i in scanners_pos.iter() {
        for j in scanners_pos.iter() {
            res = cmp::max(res, manhattan(&i, &j));
        }
    }
